...
// Interlineado 1,5 en todo el documento: OOXML no garantiza que un
// w:spacing de formato directo herede w:line del docDefault del propio
// documento (word/styles.xml) si el elemento ya trae w:before/w:after, así
// que se repite explícitamente aquí en cada w:spacing que se genera a
// mano, en vez de confiar solo en el valor por defecto de los estilos.
const LINEA_1_5 = `w:line="360" w:lineRule="auto"`;
...
function paragraphXml(text, opts) {
  ...
  } else if (!opts.sinSangria) {
    // Sangría de primera línea (formato académico estándar, ~1,25cm).
    ppr.push(`<w:ind w:firstLine="708"/>`);
  }
...
function headingXml(text, level) {
  ...
  return `<w:p><w:pPr><w:spacing w:before="240" w:after="120" ${LINEA_1_5}/><w:outlineLvl w:val="${level - 1}"/></w:pPr>${runXml(text, { bold: true, sz, color: "133A5C" })}</w:p>`;
}
...
// interlineado 1,5 como valores por defecto de TODO el documento — sin este
// archivo, Word usa su fuente/interlineado por defecto del tema
// (normalmente Calibri/Aptos a espacio simple) en cualquier texto que no
// traiga ya su propio w:rFonts/w:spacing explícito.
function buildStylesXml() {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>` +
    ...
    `</w:rPr></w:rPrDefault>` +
    `<w:pPrDefault><w:pPr>` +
    `<w:spacing w:after="160" ${LINEA_1_5}/>` +
    `</w:pPr></w:pPrDefault>` +
    `</w:docDefaults>` +
