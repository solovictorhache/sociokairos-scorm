...
**2) Selector manual de dirección VI/VD**. Hasta ahora el motor asignaba VI/VD por convención léxica y solo avisaba de la ambigüedad (`NOTA_DIRECCIONALIDAD_VIVD`). Nuevo botón «⇄ Intercambiar VI ↔ VD» junto a la sección de variables: `analizarProblema(texto, { intercambiarViVd: true })` intercambia VD/VI (y sus motivos de transparencia) antes de calcular preguntas, correlaciones, hipótesis y operacionalización — no es solo una etiqueta que cambia, todo el análisis derivado se recalcula con la nueva dirección. Área y marcos teóricos no cambian (se construyen sobre el texto combinado de ambas listas, sin importar el orden), como debe ser.

**3) Historial local de problemas trabajados**. Cada análisis exitoso se guarda en `localStorage` (solo en el dispositivo del estudiante, nunca se envía a ningún servidor) con un máximo de 8 entradas (MRU, sin duplicados por texto exacto). Nueva lista en la sección 1 con la fecha y el texto de cada problema; un clic recarga el texto y vuelve a lanzar el análisis completo, para comparar versiones sucesivas de un mismo problema mientras se va puliendo. Botón «Borrar historial» para limpiarlo. Degrada con seguridad si `localStorage` no está disponible (algunos iframes de LMS lo restringen).

**4) Campo de justificación teórica real en el Word exportado**. La nota `NOTA_JUSTIFICAR_MARCOS` ("justifica por qué este marco es pertinente") era solo un recordatorio en pantalla que nunca llegaba al informe. Ahora hay un `<textarea>` real bajo los marcos teóricos sugeridos (sección 2.4) cuyo contenido se inserta literalmente en el informe Word exportado, bajo un nuevo párrafo "Justificación teórica propia del estudiante" en la sección 6 — sin añadir una sección numerada nueva, para no romper el conteo de secciones que valida `validate_docx.py`.

Las cuatro mejoras se probaron end-to-end con Playwright en un test nuevo (`tests/e2e/mejoras.e2e.js`, añadido a CI): transparencia visible tras reformular, el swap de VI/VD recalcula P1/P2/P3 y puede deshacerse, el historial guarda y recarga una entrada disparando un reanálisis completo, y el `.docx` exportado contiene literalmente el texto de justificación escrito por el estudiante (verificado descomprimiendo el `.docx` y comprobando `word/document.xml`). 46/46 tests unitarios en verde (6 nuevos: 4 de transparencia, 2 del selector VI/VD).
